...
class Donthuoc_Controler {
  ...
  Select_Status_Donthuoc = (req , res , next) => {
    ...
    Connect_Data_Model.Select_Status_Donthuoc__M (Date , (error , result) => {
      if (error) return next (error);
      if (result.length === 0) return res.status(200).json ({message : `Không có đơn thuốc cần phát`});
      res.status(200).json ({
...
ThayDoiTrangThai = (req, res, next) => {
  ...
  Status_handling = (req , res , next) => {
    ...
    if (result[0].TrangThai ==='DaPhatThuoc') return res.status(200).json ({message : "Trạng thái đã được xác nhận trước đó" , error: err });
     if (result[0].TrangThaiThanhToan === false) return res.status(500).json ({message : "Đơn thuốc này chưa được thanh toán" , error: err });
  ...
  try {
    const result = await Connect_Data_Model.Filter_Donthuoc_ByDate_M(limit, page, all, { fromDate, toDate, year });
